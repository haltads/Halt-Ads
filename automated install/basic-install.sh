...
webInterfaceGitUrl="https://github.com/haltads/AdminLTE.git"
webInterfaceDir="${webroot}/admin"
piholeGitUrl="https://github.com/haltads/Halt-Ads.git"
...
FTLinstall() {
    ...
    if [[ "${ftlBranch}" == "master" ]];then
        url="https://github.com/haltads/FTL/releases/latest/download"
    else
...
FTLcheckUpdate() {
    ...
    if [[ ! "${ftlBranch}" == "master" ]]; then
        ...
        if [[ ${ftlLoc} ]]; then
            ...
            if ! FTLlatesttag=$(curl -sI https://github.com/haltads/FTL/releases/latest | grep --color=never -i Location: | awk -F / '{print $NF}' | tr -d '[:cntrl:]'); then
            ...
            if [[ "${FTLversion}" != "${FTLlatesttag}" ]]; then
                ...
                remoteSha1=$(curl -sSL --fail "https://github.com/haltads/FTL/releases/download/${FTLversion%$'\r'}/${binary}.sha1" | cut -d ' ' -f 1)
                localSha1=$(sha1sum "$(which pihole-FTL)" | cut -d ' ' -f 1)
    ...
    # Otherwise,
        ...
        if is_command sudo ; then
            ...
            if [[ "$0" == "bash" ]]; then
                ...
                exec curl -sSL https://raw.githubusercontent.com/haltads/Halt-Ads/master/automated%20install/basic-install.sh | sudo bash "$@"
            else
